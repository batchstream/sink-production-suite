...
| --- | --- |
| `TestStoreBackpressureKeepsSharedAdmissionBounded` | A held write consumes the shared single-slot window. Eight writes remain queue-charged, excess work is rejected, Native Count cannot bypass the budget, health remains ready, canceled queued writes never reach storage, and resource growth stays bounded. Released work and Native access recover. |
