...
| Healthy Store cold-start rejection | A ready, untrained Engine accepts a bounded concurrent Query/Count burst without retries, warmup traffic, or manufactured backend success | `TestReadyStoreAdmitsColdQueryBurst` |
| Native read admission queue | Query/Count share the Store budget without fail-fast rejection below queue limits; overflow is bounded, only caller contexts end waiting, canceled/expired reads never reach storage, and the remaining request executes once after capacity returns | `TestQueryAndCountAdmissionQueueBoundedAndCancelable` |
