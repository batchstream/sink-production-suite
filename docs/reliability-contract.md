...
| [PR 40](https://github.com/liran/sink/pull/40): coalesced byte budgets | Independent Read RPCs and duplicate addresses each return their documents in separate bounded frames, even when their combined size exceeds the message limit; oversized individual results are rejected | `TestReadStreamsUsePerResultLimits` |
