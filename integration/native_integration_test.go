...
func (f *nativeFixture) seed(t *testing.T, count int) {
	...
	results, err := f.dataset.Create(t.Context(), createRequest)
...
func TestNativeBackendExecute(t *testing.T) {
	nativeFixtures(t, func(t *testing.T, f *nativeFixture) {
		...
		readRequest := sink.DatasetReadRequest{
			Keys: []sink.Key{sink.StringKey("record-00")},
		}
		before, err := f.dataset.Read(t.Context(), readRequest)
		...
		if f.bson {
			...
			readRequest3 := sink.DatasetReadRequest{
				Keys: []sink.Key{sink.StringKey("partial-batch-success")},
			}
			persisted, err := f.dataset.Read(t.Context(), readRequest3)
...
func TestNativeBackendReturnedWrites(t *testing.T) {
	nativeFixtures(t, func(t *testing.T, f *nativeFixture) {
		...
		results, err := f.environment.client.Write(t.Context(), writeRequest)
		...
		for i := range writers {
			group.Go(func() {
				...
				writeRequest := sink.WriteRequest{
					...
					Operations:     []sink.WriteOperation{returned},
				}
		...
		failed, err := f.dataset.Create(t.Context(), createRequest)
		...
		record.Value = value
		replaceRequest := sink.DatasetWriteRequest{
			CompletionMode: sink.CompletionWaitUntilApplied,
			Records:        []sink.Record{record},
		}
		replaced, err := f.dataset.Replace(t.Context(), replaceRequest)
