...
case "${phase}" in
  local)
    ...
    required+=',TestNativeTransportSharesStoreAdmission,TestAdmissionQueueIndependentOfBatchOperationLimits,TestBatchTicketCountsTasksNotOperationsAndTransfersBytes,TestFullAdmissionQueueBackpressuresProducerWithoutRejection,TestBatchAndDirectAdmissionShareFIFO,TestReservationReleaseRacesWithTicketCancellation,TestUncontendedQueuedBatchesDoNotInventGrowthDemand,TestBatchAndNativeMethodsShareOneDispatchFIFO,TestBatchProducerWaitsForAdmissionSpaceWithoutBlockingCancellation,TestCanceledMemberOfQueuedBatchDoesNotCancelOtherCallers,TestAdmissionValidationNilAndReusedPermit,TestAdmissionMetricsSeparateWaitingAndPermitHold'
    required+=',TestHealthyStartupOpensBoundedWindow,TestAdmissionFIFOAndCancellationReleaseEveryQueuePosition,TestAdmissionByteLimitAndCapacityReuse,TestAdmissionPreservesContextAndOnlyCallerEndsWaiting,TestQueuedHealthyReadsGrowWithoutRejectingRequests,TestAdmissionWaitsThroughCooldownWithoutRevokingActiveWork,TestAdmissionCancelReleaseContentionDoesNotLeak,TestAllNativeMethodsWaitForStoreCapacityWithoutNewDeadline,TestEngineCompletesStoreStartupBeforeServingReadiness,TestEngineStartupHonorsProcessCancellation'
