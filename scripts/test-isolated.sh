...
cd "${suite_dir}"
go test -race -tags=integration ./conformance -run '^TestStoreIsolated' -count=1 -timeout=5m -json | tee "${SINK_CONFORMANCE_ARTIFACTS}/tests.jsonl"
go run ./cmd/check-test-events --file "${SINK_CONFORMANCE_ARTIFACTS}/tests.jsonl" --require 'TestStoreIsolatedGatewayPublicContract,TestStoreIsolatedWorkerColdStartWithoutEngine'
